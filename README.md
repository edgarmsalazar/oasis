...
- `mini_boxes_nside_<xx>`: contains the simulation box split into miniboxes. This is a duplication of the original data provided by the user but arranged for <span style="font-variant:small-caps;">Oasis</span>. It is recommended not to remove this directory until all subsequent runs on the same box are finished. If you want to visualise or work with minibox data use the [`load_particles`](https://github.com/edgarmsalazar/oasis/blob/dev/oasis/minibox.py#L379) and [`load_seeds`]([`load_particles`](https://github.com/edgarmsalazar/oasis/blob/dev/oasis/minibox.py#L449)) methods from [`oasis.minibox`](oasis/minibox.py) module.
